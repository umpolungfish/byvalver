...
## Overview
Updated `verify_denulled.py` to support bad-character profiles matching byvalver's `--profile` option, providing a consistent verification workflow for the generic bad-character elimination framework.

## Changes Made

### 1. Profile Support
- **Added 13 bad-character profiles** matching those in `src/badchar_profiles.h`:
...
- `--profile NAME`: Use a predefined bad-character profile
...
- **Backward compatibility** maintained - existing `--bad-chars` option still works

### 4. Bug Fixes
- **Fixed consecutive bad character counting**: Previously, consecutive bad characters (e.g., `\x00\x0a`) were counted as a single bad character. Now each byte is counted individually with correct position tracking.
...
```bash
# Manual bad-char specification (still works as before)
...
python3 verify_denulled.py --bad-chars "00,0a" -r input_dir/ output_dir/
```

## Workflow Integration

The updated tool now integrates seamlessly with byvalver's generic bad-character elimination:
...
### Profile Definitions
Profiles are defined in `BADCHAR_PROFILES` dictionary at the top of `verify_denulled.py`, mirroring the C definitions in `src/badchar_profiles.h` to ensure consistency.

### Profile Precedence
When both `--profile` and `--bad-chars` are specified, `--profile` takes precedence (with a warning in the help text).
...
- **Fully backward compatible** - existing scripts using `--bad-chars` continue to work
...
- Manual `--bad-chars` specification still works
- Batch processing works with both profiles and manual specification
- Consecutive bad character bug fix validated
...
- **v3.0.3** (2025-12-19): Added profile support and fixed consecutive bad character counting bug
- **v3.0** (2025): Added generic bad character elimination support
