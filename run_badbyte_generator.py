#!/usr/bin/env python3
"""
BYVALVER Bad-Byte Removal Technique Generator
=============================================
Runs a four-stage agent pipeline to:
  1. Discover existing bad-byte removal strategies
  2. Propose a novel bad-byte elimination technique
  3. Generate complete C implementation
  4. Write files, patch registry, and verify build

Usage:
    python run_badbyte_generator.py [options]
"""
import asyncio
import argparse
import json
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description="BYVALVER Bad-Byte Removal Technique Generator",
    ...
    _banner("BYVALVER Bad-Byte Removal Technique Generator")
    ...
    discovery_result = await discovery_agent.run(
        task="Catalog existing bad-byte strategies",
        context={"mode": "bad_byte", "arch": args.arch},
    )
...
ERROR: Discovery failed — {discovery_result.get('error')}", file=sys.stderr)
    ...
    proposal_result = await proposal_agent.run(
        task=f"Propose a novel bad-byte elimination strategy for {args.arch}",
        context={
            "mode": "bad_byte",
...
ERROR: Proposal failed — {proposal_result.get('error')}", file=sys.stderr)
    ...
    codegen_result = await codegen_agent.run(
        task="Generate C implementation for the proposed technique",
        context={
            "mode": "bad_byte",
            "proposal": proposal,
            "catalog": discovery_result.get("catalog", {}),
...
ERROR: Code generation failed — {codegen_result.get('error')}", file=sys.stderr)
    ...
    if impl_result.get("build_success"):
        print(f"  SUCCESS: Bad-byte removal technique implemented!")
    else:
