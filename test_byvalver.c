...
// Test 7: ADD EAX, 0x00100000 (testing advanced arithmetic strategies)
test_result_t test_add_eax_large_imm() {
    test_result_t result = {.name = "ADD EAX, 0x00100000", .passed = 0, .error_msg = NULL};

    // 05 00 00 10 00 = add eax, 0x00100000
    uint8_t shellcode[] = {0x05, 0x00, 0x00, 0x10, 0x00};
...
// Test 8: MOV EBX, 0x00200000 (testing register other than EAX)
test_result_t test_mov_reg_large_imm() {
    test_result_t result = {.name = "MOV EBX, 0x00200000", .passed = 0, .error_msg = NULL};

    // bb 00 00 20 00 = mov ebx, 0x00200000
    uint8_t shellcode[] = {0xbb, 0x00, 0x00, 0x20, 0x00};
...
// Test 9: XOR encoding strategy
test_result_t test_xor_encoding() {
    test_result_t result = {.name = "XOR Encoding Strategy", .passed = 0, .error_msg = NULL};

    // Test with a value that should be encoded via XOR
    uint8_t shellcode[] = {0xb8, 0x00, 0x11, 0x00, 0x00}; // mov eax, 0x00110000
...
// Test 10: NEG encoding strategy
test_result_t test_neg_encoding() {
    test_result_t result = {.name = "NEG Encoding Strategy", .passed = 0, .error_msg = NULL};

    // Test with a value that should be encoded via NEG (like 0x00730071)
    uint8_t shellcode[] = {0xb8, 0x71, 0x00, 0x73, 0x00}; // mov eax, 0x00730071
...
// Test 11: Complex multi-instruction sequence
test_result_t test_complex_sequence() {
    test_result_t result = {.name = "Complex Multi-Instruction Sequence", .passed = 0, .error_msg = NULL};

    // Multiple instructions with null bytes
    uint8_t shellcode[] = {
        0xb8, 0x00, 0x00, 0x00, 0x00,  // mov eax, 0x00000000
        0xbb, 0x00, 0x10, 0x00, 0x00,  // mov ebx, 0x00001000
        0x05, 0x00, 0x00, 0x10, 0x00,  // add eax, 0x00100000
        0x90                           // nop
    };
...
int main() {
    ...
    test_result_t tests[] = {
        ...
        test_push_zero(),
        test_no_nulls(),
        test_add_eax_large_imm(),
        test_mov_reg_large_imm(),
        test_xor_encoding(),
        test_neg_encoding(),
